window.APP_THEME_SETS = {
    logs: ["cloud", "chrome", "mocha", "mag", "wheatgrass", "scarab"],
    ostinuto: ["chrome"], // example
  };
  
  window.getThemesForCurrentApp = function () {
    const appId = window.__APP_ID__ || "logs";
    return window.APP_THEME_SETS[appId] || window.APP_THEME_SETS.logs;
  };
  
  // optional: if you want different defaults per app
  window.getDefaultThemeForCurrentApp = function () {
    const appId = window.__APP_ID__ || "logs";
    const defaults = { logs: "cloud", ostinuto: "chrome" };
    return defaults[appId] || "cloud";
  };

  window.getThemeStorageKeyForCurrentApp = function () {
    const appId = window.__APP_ID__ || "logs";
    return `selectedTheme:${appId}`;
  };
...
function getAppAssetBase() {
    switch (window.__APP_ID__) {
      case "logs":
        return "/logs/images";
      case "ostinuto":
        return "/ostinuto/images";
      case "home":
      default:
        return "/images/Home";
    }
...
function applyTheme(themeName) {
    ...
    if (logoImg) {
        ...
        // Use assetHref from components.js if available for consistent path construction
        if (typeof window.assetHref === 'function') {
            // Get app asset base without leading slash for assetHref
            const appId = window.__APP_ID__ || 'logs';
            const assetBase = appId === 'logs' ? 'logs/images' : appId === 'ostinuto' ? 'ostinuto/images' : 'images';
            logoImg.src = window.assetHref(`${assetBase}/logo_${logoColor}.PNG`);
        } else {
            // Fallback to getAppAssetBase (which returns path with leading slash)
            logoImg.src = getAppAssetBase() + `/logo_${logoColor}.PNG`;
        }
     }
...
function initializeTheme() {
    const allowed = window.getThemesForCurrentApp?.() || ["cloud"];
    ...
    const themeToApply = (saved && allowedSet.has(saved)) ? saved : fallback;
